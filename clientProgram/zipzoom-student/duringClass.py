...
class ClassHandle:
    def __init__(self, studentId, classId):
        ...
        sio.on('get_point', self.pointHandler)
        sio.on('get_badge', self.badgeHandler)
        sio.on('get_ox_quiz', self.oxQuizHandler)
        sio.on('get_choice_quiz', self.choiceQuizHandler)
        sio.on('quiz_timeout', self.quizTimeoutHandler)
    ...
    def run_camera(self):
        ...
        with pyvirtualcam.Camera(width, height, fps_out, fmt=PixelFormat.BGR, print_fps=args.fps) as cam:
            ...
            while True:
                ...
                # 얼굴 눈 찾기 + 사각형 그리기
                frame = self.detect(face_cascade, eye_cascade, gray, frame)

                # 제스처 분석 -> 퀴즈가 시작되면 제스처 분석 시작, 퀴즈 끝나면 제스처 분석 끝
                if self.quizOn == True and hand_detect == False:
                    hand_detect = True
                elif self.quizOn == False and hand_detect == True:
                    hand_detect = False
